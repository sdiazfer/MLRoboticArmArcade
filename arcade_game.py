...
class RobotArm:
    def __init__(self, num_segments=3, segment_length=50):
        self.base_position = [200, 200]  # Base position at the center of the canvas
    ...
    def check_for_prize(self, prize_area):
        # Check if the end effector is within the prize area
        end_effector = self.get_joint_positions()[-1]
        x, y = end_effector
        if prize_area[0] <= x <= prize_area[2] and prize_area[1] <= y <= prize_area[3]:
            self.has_prize = True
...
class ArcadeGame:
    ...
    def generate_prize_location(self):
        # Generate a random position for the prize area within the canvas boundaries
        x = random.randint(50, self.canvas_size - 70)  # Adjusted to prevent the prize from going off-canvas
        y = random.randint(50, self.canvas_size - 70)
        size = 20  # Size of the prize area
        prize_area = (x, y, x + size, y + size)
        print(f"New prize location: {prize_area}")
        return prize_area
